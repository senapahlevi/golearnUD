...
func UpdateRole(c *fiber.Ctx) error {
	...
	// var result interface{} //ori

	// database.DB.Table("role_permissions").Where("role_id", id).Delete(&result) ori
	// database.DB.Table("role_permissions").Where("role_id", id).Delete("") //on udemy
	// database.DB.Exec("DELETE FROM role_permissions WHERE role_id = ?", id) //on udemy
	//but all of them deletes coz not working
	role := models.Role{
		Id:          uint(id),
		Name:        roleDto["name"].(string),
		Permissions: permissions,
